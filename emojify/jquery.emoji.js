...
},
{
    ...
    height: "50px",
    visibility: true,
    placeholder: "∫hot‖{alias}‖",
    emoji: {
        "知识增加": "热词系列-知识增加",
        "知识增加-虎年": "热词系列-知识增加-虎年",
        "知识增加-圣诞": "热词系列-知识增加-圣诞",
        "知识增加-春节": "热词系列-知识增加-春节",
        "知识增加-愚人节": "热词系列-知识增加-愚人节",
        "扬名立万": "热词系列-扬名立万",
        "红红火火": "热词系列-红红火火",
        "新年快乐": "热词系列-新年快乐",
        "新年快乐-旧": "热词系列-新年快乐-旧",
        "你币有了": "热词系列-你币有了",
        "你币有了-春节": "热词系列-你币有了-春节",
        "吹爆": "热词系列-吹爆",
        "吹爆-春节": "热词系列-吹爆-春节",
        "好耶": "热词系列-好耶",
        "好耶-春节": "热词系列-好耶-春节",
...
(function($, window, document) {
    ...
    Plugin.prototype = {
        _init: function() {
            ...
            var panel, name, path, maxNum, file, placeholder, emoji, index, notation;
            for (var i = 0; i < groupLength; i++) {
                if (iconsGroup[i].visibility) { // 部分博主专用表情不可输入
                    name = iconsGroup[i].name || 'group' + (i + 1);
                    path = basePath + '/' + iconsGroup[i].path;
                    maxNum = iconsGroup[i].emoji.length; // 表情最大数
                    file = iconsGroup[i].file;
    ...
    function fn(option) {
        ...
        return this.each(function() {
            ...
            var options = $.extend({},
            DEFAULTS, $this.data(), typeof option === 'object' && option);
...
function emojiParse(content, basePath, iconsGroup) {
    ...
    var groupLength = iconsGroup.length;
    var path, file, placeholder, emoji, pattern, regexp;
    if (groupLength > 0) {

        for (var i = 0; i < groupLength; i++) {
            path = basePath + iconsGroup[i].path;
            imgheight = iconsGroup[i].height;
            file = iconsGroup[i].file;
            ...
            if (emoji) {
                ...
                content.html(content.html().replace(regexp,
                function($0, $1) {
                    var n = emoji[$1];
                    if (n) {
                        return '<img class="wp-smiley" style="height:'+imgheight+';width:auto" src="' + path + n + file + '" title="' + $1 + '" alt="' + $1 + '"/>';
                    } else {
                        return $0;
                    }
                }));
